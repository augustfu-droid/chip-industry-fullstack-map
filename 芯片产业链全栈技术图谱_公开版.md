...
> 版本 v1.10-public · 初版 2026-06-15 · 维护 2026-08-09
...
### 本次维护快照（截至 2026-08-09）

| 层级 | 已核实更新 | 应如何使用 |
|---|---|---|
| **事实 F** | TSMC N2 已于 2025 年第四季度进入量产 [F-66]；ASML 截至 2025 年末累计交付 8 套 High-NA 系统 [F-67]；SK hynix 已于 2026 年 6 月送样 12 层 HBM4E [F-68]；四家云厂商最新 2026 公司级资本开支指引机械合计约 6,950–7,200 亿美元 [G-2] [G-11] [G-12] [G-15] | 仅代表各公司披露口径，样品、设备交付与资本开支都不能直接等同于客户认证、收入或利润 |
| **判断 J** | 2026 年产业主线已从“有没有技术路线”转向“能否按期认证、量产并转化为现金流”；HBM、先进封装、电力与网络仍是系统瓶颈，但瓶颈会在季度间迁移 | 同时跟踪客户验收、良率、库存、在建工程、折旧和自由现金流，不以单一发布会或总 Capex 作结论 |
| **复核点** | High-NA 的客户量产插入、HBM4E 的资格认证、N2/N2P/A16 的良率与爬坡、BIS 许可证实践、云厂商 Capex 指引变化 | 每季用公司财报、监管文件和标准组织原文更新；旧路线图若无新证据，降级为公司目标或研究假设 |
...
HBM4 规范（JEDEC JESD270-4，2025 年 4 月正式发布）[15-2]代表了下一代里程碑：接口位宽升至 2048-bit，首批产品面向更高容量和超过 1.6TB/s 的单堆栈带宽。HBM4 最重要的架构创新是基底逻辑 die（Base Die）从传统的成熟制程（20-28nm）升级至先进逻辑工艺，提供更强的内存控制、内建自测试（BIST）和错误纠正（ECC）能力[15-3,15-5]。HBM4E 的规格与堆叠形态仍带有明显的客户定制属性；截至 2026 年 8 月，最强的一手量化证据是 SK hynix 已向主要客户送出 12 层、48GB、16Gbps/pin 的 HBM4E 样品，而不是某个行业统一的 16 层量产配置[15-13]。
...
#### 15.5　2026 状态更新：HBM4E 样品与 SPHBM4 标准

SK hynix 于 2026 年 6 月 18 日宣布，已向主要客户送出 12 层 HBM4E 样品：容量 48GB，单引脚速率 16Gbps，并继续采用 Advanced MR-MUF 堆叠工艺；公司披露热阻较上一代方案降低 17%[15-13]。这是一项重要的工程里程碑，但“送样”只证明器件已进入客户测试，仍需经过可靠性、系统兼容性、良率、供货与成本验证，不能写成已经完成客户资格认证或高量产制造。
...
Apple Silicon（M4 Ultra，2025）集成 32 个性能核与 32 个效能核，Neural Engine 处理能力达 38 TOPS，统一内存最高 192GB，内存带宽 546 GB/s[21-5]。MacBook Pro M4 Max 在 LLM 本地推理（llama.cpp）测试中，70B 参数模型推理速度约 25 token/s，已具备实用性。高通 Snapdragon X Elite（X1E-80-100）采用自研 Oryon CPU 架构，12 核设计，搭配 45 TOPS Hexagon NPU，是 Windows on Arm 生态中性能最强的方案，在 SPEC CPU2017 整数测试中与 Intel Core Ultra 7 相当[21-6]。
...
[21-5] Apple Inc. Apple M4 Chip Specifications. Apple Newsroom. 2024. https://www.apple.com/newsroom/2024/05/apple-introduces-m4-chip/
...
Intel Gaudi 3（2024 年）基于台积电 N5 工艺，集成 64 个矩阵乘法引擎（MME）和 8 个张量处理器核（TPC），BF16 算力 1.835 PFLOPS，搭载 128GB HBM2e（6 叠），内存带宽 3.7 TB/s[22-6]。相较 H100，Gaudi 3 在 LLM 训练的 tokens/dollar 指标上号称具备约 40% 优势，主要来自更低的销售价格（Gaudi 3 约 25,000 美元/卡，H100 约 30,000-35,000 美元/卡）。Gaudi 3 的以太网互连架构（24 个 100GbE 端口）是其区别于 NVIDIA NVLink 私有互连的重要特征，有利于与标准网络设备集成，但互连带宽和延迟均逊于 NVLink 5。

oneAPI 是 Intel 的统一编程模型，基于 SYCL（异构 C++ 标准），试图覆盖 CPU（Xeon）、GPU（Arc/Gaudi）、FPGA（Stratix）的统一开发体验。然而面对 CUDA 10 年积累的算子库、工具链和开发者惯性，Intel 的单芯片算力不足与 oneAPI 生态碎片化问题叠加，Gaudi 在企业市场的渗透率仍有限，2024 年市占率估计低于 3%[22-12]。Falcon Shores（预计 2025 年底）将整合 GPU + Gaudi 架构于同一产品线，是 Intel AI 加速器的重要整合赌注。Intel 2024 年宣布 Gaudi 3 OAM 模块可以 8 卡构成 OAM 集群（类似 NVIDIA HGX H100 形态），试图降低云厂商的采购门槛，但在顶级超大规模客户（Hyperscaler）的实际部署中仍处于试点阶段。
...
Google TPU（Tensor Processing Unit）是最具代表性的 AI ASIC，发展至今已历经八代迭代。TPU v5p（2023 年末）是谷歌迄今规模最大的训练加速器：单芯片 1.8 PetaFLOPS（BF16），搭载 96GB HBM2e，通过 Optical Circuit Switch（OCS）互连，8960 块 TPU 可组成一个 Pod，总算力约 16 ExaFLOPS（BF16）[23-3]，专为 1 万亿参数以上模型训练设计。

TPU v6（代号 Trillium，2024 年）在 v5e 基础上大幅提升峰值算力（约 4.7 倍）和内存带宽（约 2 倍），能效比提升约 67%[23-1]，是谷歌 Gemini 超大模型训练的主力基础设施。Trillium 采用台积电 N4P 工艺，支持最大 256 块的高速互连 Pod，用于大规模并行训练的全 reduce 延迟得到显著优化。Ironwood（TPU v7，2025 年预发布）是 Google 首款面向推理优化的 TPU，单 Pod 算力约 42.5 ExaFLOPS，搭载 192GB HBM[23-2]，主要目标是 Gemini 系列模型的大规模商业推理部署，cost-per-token 目标较 v5p 降低约 80%。TPU 的互连技术路线（ICI，Inter-Chip Interconnect）区别于 NVIDIA NVLink：TPU 使用定制光电交换网络（OCS），可实现数千芯片的低延迟互连，在超大规模 Pod 训练时集合通信效率高于 NVIDIA DGX 集群约 10%-15%。
...
Intel Loihi 2（2021 年，Intel 4 工艺，即 Intel 自研 7nm 级别）集成 100 万个神经元（Graded Neuron，支持多层时空计算）和 1.2 亿个突触，功耗约 1W[26-1]，已作为研究平台向全球大学和研究机构开放。Loihi 3（规划 2026 年量产，台积电 4nm 工艺）将集成 800 万神经元和 640 亿突触，功耗目标 1.2W[26-2]，面积效率提升约 8 倍。在特定基准任务（嗅觉识别、深度传感器处理）上，Loihi 2 的能效比 GPU 高出 3-4 个数量级，但适用任务范围极为有限。
...
[26-2] Intel Labs. Intel Neuromorphic Research Community: Loihi 3 Roadmap. Intel Labs. 2024. https://www.intel.com/content/www/us/en/research/neuromorphic-computing.html
...
2022 年 10 月的出口管制规则是战略性转折点：BIS 以先进计算性能与互连能力等参数限制相关芯片向中国出口，同时扩大半导体制造设备和最终用途管制，并通过外国直接产品规则（FDPR）将部分依赖美国软件或技术生产的非美国物项纳入管辖[36-4]。2023 年 10 月，BIS 再次调整先进计算芯片参数并扩大设备、最终用途和规避交易覆盖范围，使专为绕开旧阈值设计的产品也可能落入许可要求[36-5]。
...
**主线 B（AI 算力基础设施）**：全球 AI 训练与推理需求驱动数据中心 Capex 快速扩张。附录 G 根据五家公司年报列出 2025 已披露的 Capex/PP&E 现金口径，并单列 Alphabet、Meta 与 Amazon 的 2026 公司级指引。这些数字包含服务器、网络、建筑及部分非 AI 资产，且财年与租赁口径不同，不宜直接当作 HBM、先进封装或光模块的可获得市场。产业观察应继续下钻到机柜上架、加速器与自研 ASIC 混合比、HBM 单机用量、网络拓扑、供应商份额、良率与收入确认时滞。
...
#### B.1　制造（先进逻辑制程）证据状态表（截至 2026-08-09）
...
| Samsung SF2 及以下 | 2026 年 7 月，Samsung 与 Broadcom 签署聚焦 2nm 及以下、存储、代工和封装协作的 MOU [F-70] | Samsung 先前路线图将 SF2 分别面向移动、HPC 和汽车应用，但均属公司目标 [F-7] | [A] MOU 不是订单、客户资格认证或量产证明；等待产品 Tape-out、量产和收入证据 |
...
| 2026-06 | SK hynix 向主要客户送出 12 层、48GB、16Gbps/pin HBM4E 样品 [F-68] | 服务器 DRAM 与 NAND 仍需结合产品组合、库存和合约价判断，不能由 HBM 景气直接外推 | [F] 样品已送出；下一步看资格认证、可靠性、良率和收入 |
...
- AMD 每年 11 月前后举办 Financial Analyst Day 或 EPYC 服务器新品发布会；Samsung SDC、Qualcomm Snapdragon Summit 通常也集中在秋季，值得一并跟踪。
...
> 本附录收录 71 条核心参考文献，按技术领域分类，格式为 `[F-K] 作者/机构. 标题. 来源. 年份. <URL>`。其中 F-66 至 F-71 为 2026-08-09 维护时新增的一手来源；“收录”只代表可追溯，不代表链接永久有效或所有数据跨期可比。
...
> **口径声明（截至 2026-08-09）**：本附录是研究方法，不是证券推荐。「事实」必须有可追溯来源与期间；「假设」必须可修改；「判断」必须写出证伪条件。估值倍数、产能时滞、BOM 占比与库存阈值都不是跨公司通用常数，不再使用无期间、无样本的「经验区间」作结论。
...
|---|---|---:|---|---:|---|
| Microsoft | FY2025，截至 2025-06-30 | 64.551 | additions to property and equipment [G-1] | 约 190 十亿美元 [G-11] | 指引为 CY2026 公司口径并含约 25 十亿美元组件价格影响；财年与自然年不同 |
| Alphabet | CY2025 | 91.4 | capital expenditures [G-2] | 175–185 十亿美元 [G-2] | 主要是技术基础设施，但不等于纯 AI 芯片采购 |
...
| Amazon | CY2025 | 128.32 | cash capital expenditures，净额 [G-4] | 约 200 十亿美元 [G-15] | 同时包含 AWS 与履约网络，不能全部归因于 AI |
| Oracle | FY2025，截至 2025-05-31 | 21.2 | cash used for capital expenditures [G-5] | 本表不填无同口径一手值的数 | 财年错位，需与后续披露滚动衔接 |

五个 2025 披露数机械相加约为 **3,751.6 亿美元**，但它只是不同财年、不同现金/应付/租赁口径的「规模感」，不应标注为「五大云厂商 AI Capex」。Microsoft、Alphabet、Meta 与 Amazon 四家最新 2026 公司级指引机械合计约为 **6,950–7,200 亿美元**；这不包含 Oracle，且各公司定义、披露时点和非 AI 支出范围不同，更不能直接当作半导体可获得市场。

##### G.2.2 2026 最新披露：分母变大，更要检查现金与利润

| 公司 | 最新披露期间 | 已披露指标 | 同期前瞻 / 财务含义 | 口径限制 |
|---|---|---|---|---|
| Microsoft | FY2026 Q3 | 季度 Capex 31.9 十亿美元；其中现金 PP&E 30.9 十亿美元，约三分之二投向 GPU/CPU 等短寿命资产 [G-11] | 公司预计下一季度 Capex 超过 40 十亿美元；短寿命资产占比高，意味着折旧传导会更快 | 季度数不能直接年化；公司 CY2026 指引与财年报表错位 |
| Meta | CY2026 Q2 | Capex（含融资租赁本金）31.08 十亿美元 [G-12] | 全年指引收窄至 130–145 十亿美元，需求信号更强，但未来折旧和运营费用也需同步上修 | 包含租赁本金，不能与纯现金购建 PP&E 直接比较 |
| Amazon | 截至 2026 Q2 的 TTM | 购建 PP&E 净额 169.007 十亿美元；自由现金流为 -7.604 十亿美元 [G-13] | 公司称增量主要反映 AI 投资；同时说明 Capex 强度已经显著压缩当期 FCF | TTM 不是单季；同时包含 AWS 与履约网络 |
| TSMC | CY2026 Q2 | 收入 40.20 十亿美元，毛利率 67.7%，营业利润率 60.3% [G-14] | Q3 收入指引 44.6–45.8 十亿美元、毛利率 65%–67%；可作为 AI 基础设施需求向制造端传导的交叉验证 | TSMC 是供应链承接方，不应与云厂商 Capex 合计 |
| Alphabet | 最新可核实公司级指引 | 本版继续采用 2025 Q4 发布的 2026 Capex 指引 175–185 十亿美元 [G-2] | 下一次更新应以更晚的 Alphabet 官方财报原文替换 | 本轮维护未取得可替换 [G-2] 的同口径公司原文，明确保留为待更新项 |

这些披露共同支持“AI 基础设施投入继续上行”，但也同时抬高折旧、现金消耗与执行风险。研究结论只有在供应商收入、毛利和现金回收同步改善时才算闭环；若只是 Capex 扩大而 FCF、在建工程周转或供应商库存恶化，不能把投入本身当作价值创造。

##### G.2.3 Capex 传导的可审计公式
...
[G-11] Microsoft. *FY 2026 Third Quarter Earnings Conference Call*. Microsoft Investor Relations. 2026. <https://www.microsoft.com/en-us/investor/events/fy-2026/earnings-fy-2026-q3>
...
| H-11 | 五家公司 2025 已披露 Capex/PP&E 口径机械合计；四家公司 2026 指引机械合计 | 约 3,751.6 亿美元；约 6,950–7,200 亿美元 | Microsoft / Alphabet / Meta / Amazon / Oracle 年报、最新财报与附录 G 逐项表 | 一手 + 汇总 | 混合财年且含非 AI 支出；四家公司指引的定义与披露时点也不同，均不能标注为「AI Capex」或半导体 TAM |
...
| H-22 | SK hynix HBM4E 工程进度 | 12 层、48GB、16Gbps/pin 样品已于 2026-06 送交主要客户 | SK hynix Newsroom [15-13] / [F-68] | 一手 | 送样不等于资格认证或 HVM，需继续验证可靠性、良率和收入 |
...
4. 本表 H-12 至 H-14（华为韬定律相关）建议每季度复核，关注量产芯片独立测试结果与论文公开细节；H-21 至 H-24 建议随季度财报、客户认证和 BIS 新规滚动复核。
...
- 重写附录 B 的先进制程与存储路线图，使用 [F] 已发生事实、[T] 公司目标、[A] 研究假设三级证据状态，删除无来源的精确季度预测
