...
华为海思在2020至2023年的技术文献中隐性提出了"陶定律"（Tao's Law）——即系统级算力密度的提升不必依赖单点制程节点推进，而可通过异构互连架构、专用计算单元的高效组合来等效实现[0-11]。这与imec的CMOS 2.0思路在本质上一脉相承，但路径选择截然不同：华为更依赖自研的封装技术和计算架构弥补制程劣势，而imec的路径更依赖材料与工艺创新。两者的张力折射出全球半导体创新在制裁约束下的分叉态势。
...
#### 36.5　"陶定律"与中国特色半导体演进路径

华为首席执行官任正非和技术专家群体提出的"陶定律"概念（以华为半导体战略负责人陶景文名字命名），代表了中国在先进工艺受阻后寻求系统性突破的独特路径。陶定律的核心思路是：在无法获得最先进制程（如 EUV 3nm）的情况下，通过以下手段在系统层面逼近同等算力表现：逻辑折叠（3D Stacking）——将多个成熟工艺芯片通过先进封装叠加，等效提升计算密度，例如将两块 7nm 芯片通过 CoWoS 堆叠，在某些工作负载下逼近 3-4nm 单芯片的性能；系统级优化——针对特定应用场景（如大模型推理）做深度软硬件协同优化，放弃通用性换取专用效率；以及工艺节点内的极限挖掘——在 7nm（SMIC N+2 节点）工艺框架下通过多重图形、全场景 EPC 优化尽可能提升集成密度 [36-17]。

华为 Kirin 9010 采用 SMIC 改进 7nm 工艺，实现了在 Mate 60 Pro 上 5G 通信能力的回归，验证了这一路径的可行性 [36-18]。这对全球半导体格局的深层启示是：出口管制虽能延缓中国先进制程的追赶速度，但无法完全阻断其在系统效率上的持续迭代，特别是在 AI 推理等对"通用算力"需求相对有限的场景。投资者需要警惕一种常见的误读：将工艺代际差异（如中国量产 7nm 对比台积电 3nm）直接等同为最终应用性能差异——在特定应用场景和封装技术加持下，这一差距可能比工艺代数差距所暗示的更小。
...
[36-17] SemiAnalysis. Huawei's Tao's Law: How China Plans to Overcome Chip Sanctions. SemiAnalysis. 2024. <https://www.semianalysis.com/p/huaweis-tao-law>
